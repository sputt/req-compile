...
load("//private:reqs_repo.bzl", "BUILD_FILE_TEMPLATE", "RULES_PYTHON_COMPAT", "generate_interface_bzl_content", "process_lockfile", "write_defs_file")
...
def _reqs_hub_impl(repository_ctx):
    repository_ctx.file("BUILD.bazel", BUILD_FILE_TEMPLATE.format(
        packages = json.encode_indent(sorted(depset(repository_ctx.attr.packages).to_list())),
    ))
    for defs_id in repository_ctx.attr.defs:
        defs_file = repository_ctx.path("defs_{}.bzl".format(defs_id))
        write_defs_file(repository_ctx, repository_ctx.attr.packages, defs_file, defs_id, name = repository_ctx.attr.hub_name)

    repository_ctx.file("requirements.bzl", RULES_PYTHON_COMPAT.format(
        repository_name = repository_ctx.name,
    ))
    repository_ctx.file("defs.bzl", generate_interface_bzl_content(repository_ctx.attr.defs, repository_ctx.name))
                ...
                for repo_name, data in packages.items():
                    ...
                    if _is_wheel(data):
                        ...
                        whl_repository(
                            ...
                            package = name,
                            reqs_repository_name = parse.name,
                            whl_data = Label("{}~{}_sdist//:whl.json".format(extension_namespace, name)),
                            version = data["version"],
                        )

            reqs_hub(name = parse.name, hub_name = parse.name, defs = defs, packages = all_packages)
...
_parse = tag_class(
    doc = "Parse a lock file into a hub repo.",
    attrs = {
