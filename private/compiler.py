...
def parse_args(args: Optional[Sequence[str]]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no_index",
        action="store_true",
        help="If set, all indexes will be ignored. This will require the solution file to already satisfy all requirements.",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def compile_requirements(
    ...
    for name, input_file in requirements_ins.items():
        ...
        find_links = find_links.union(
            [os.path.normpath(input_file.parent / link) for link in new_links]
        )
    ...
    repo = build_repo(
        solutions=[str(solution)] if solution else [],
        upgrade_packages=[],
        sources=[],
        excluded_sources=[],
        find_links=sorted(find_links),
        index_urls=sorted(
...
def relativize(from_path: Path, to_path: Path) -> Path:
    ...
    """
    if from_path.is_relative_to(to_path):
        relative = from_path.relative_to(to_path)
        return Path("../" * len(relative.parents))
...
def compile_main(args: argparse.Namespace, runfiles: Runfiles) -> None:
    ...
    try:
        result = compile_requirements(
            ...
            solution=None if args.upgrade else solution,
    ...
    if args.output:
        ...
        # TODO: Have req-compile properly handle relativizing find-links paths.
        links_relative = relativize(output.parent, workspace_dir)
        output.write_text(
            buffer.getvalue().replace(str(workspace_dir), str(links_relative)),
            encoding="utf-8",
        )
