...
BUILD_FILE_TEMPLATE = """\
load(":defs.bzl", "pkg", "requirement", "requirement_wheel")
...
def requirement(name):
    return "@@{repository_name}//:{{name}}".format(
        name=name,
    )
...
def generate_interface_bzl_content(defs, repository_name):
    """Generate the content of a bzl module as an interface into the requirements.

    Args:
        defs: Def mapping, id to constraint.
        repository_name: The repository name containing this interface bzl.

    Returns:
        The content of the bzl.
    """
    loads = []
    all_requirements = []
    all_requirements_wheels = []
