...
def parse_constraint(data, repository_name, lockfile, wheel_dirs):
    ...
    Args:
        ...
        lockfile (Label): The label of the current lockfile.
    ...
    if not url.startswith(("http://", "https://", "file://")):
        if wheel_dirs and url.startswith(*wheel_dirs):
            if url.startswith(("..", "./../")):
                ...
                whl = str(Label("{}//{}:{}".format(repository, new_package, wheel)))
            else:
...
def _process_lockfile(repository_ctx, requirements_lock, constraint = None):
...
def _py_requirements_repository_impl(repository_ctx):
    ...
    _write_defs_file(repository_ctx, packages, repository_ctx.path("defs.bzl"))
...
def whl_repo_name(package):
    fail("py_multi_plat_reqs_repository cannot determine the correct repo name at analysis time for: " + package)
...
def _generate_interface_bzl_content(defs, repository_name):
    loads = []
    all_requirements = []
    all_requirements_wheels = []
    entry_point = []
...
def _py_multi_plat_reqs_repository_impl(repository_ctx):
    all_packages = []
    defs = {}
...
py_multi_plat_reqs_repository = repository_rule(
    doc = """\
An expansion on [py_requirements_repository](#py_requirements_repository) which defines targets for multiple platforms at once.

Each lock file provided will create a dependency tree matching `{name}_{id}__{package}` where `id` is the name of the lock file
with `[".", "-"]` characters are replaced by `"_"` and the name `requirements` will be stripped. This means each `requirements_lock`
entry will need to be somewhat uniquely named.
""",
    implementation = _py_multi_plat_reqs_repository_impl,
    attrs = {
        "annotations": attr.string_dict(
            doc = (
                "Optional annotations to apply to packages. For details see " +
        ...
        "interpreter": attr.label(
            doc = (
                "The label of a python interpreter to use for compiling source distributions (sdists). If " +
                "this is not provided then the provided lock files must represent binary-only dependencies."
            ),
            allow_files = True,
        ),
        "requirements_locks": attr.label_keyed_string_dict(
            doc = (
                ...
                "This requirements files _must_ be produced by [req-compile](https://pypi.org/project/req-compile/) and include " +
                "hashes and urls in the solution."
            ),
            allow_files = True,
            allow_empty = False,
            mandatory = True,
