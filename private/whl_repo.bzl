...
def _whl_repository_impl(repository_ctx):
    ...
    if repository_ctx.attr.whl or repository_ctx.attr.whl_data:
        if repository_ctx.attr.whl:
            if repository_ctx.attr.urls or repository_ctx.attr.sha256 or repository_ctx.attr.whl_data:
        ...
        execute(repository_ctx, ["cp"] + cp_args + [whl_file, repository_ctx.path(whl_name)])
    else:
